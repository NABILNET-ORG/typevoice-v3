...
impl HistoryManager {
    ...
    pub fn get_audio_file_path(&self, file_name: &str) -> Result<PathBuf, String> {
        use std::path::Path;
        ...
        // Use Path to extract the file_name component and ensure it matches the input exactly.
        // This prevents directory traversal including .., /, \, and UNC paths on Windows.
        let path = Path::new(file_name);
        let extracted = path
            .file_name()
            .and_then(|f| f.to_str())
            .ok_or_else(|| "Invalid file name: not a valid file name component".to_string())?;

        if extracted != file_name {
    ...
    pub async fn delete_entry(&self, id: i64) -> Result<()> {
        ...
        if let Some(entry) = self.get_entry_by_id(id).await? {
            // Delete the audio file first (DB filenames are trusted, but validate anyway)
            match self.get_audio_file_path(&entry.file_name) {
                ...
                Err(e) => {
                    error!(
                        "Invalid file name in database entry {}: {}",
                        entry.file_name, e
                    );
                }
            }
