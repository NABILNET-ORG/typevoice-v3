...
pub fn change_post_process_api_key_setting(
    ...
    // Try keyring first, fall back to settings store
    match crate::keyring_store::set_api_key(&provider_id, &api_key) {
        Ok(()) => {
            // Clear from settings store if it was there
            let mut settings = settings::get_settings(&app);
            if settings
        ...
        Err(e) => {
            log::warn!("Keyring unavailable, falling back to settings store: {}", e);
            let mut settings = settings::get_settings(&app);
...
pub async fn fetch_post_process_models(
    ...
    // Get API key (keyring first, then settings store fallback)
    let api_key = settings::get_api_key_for_provider(&app, &provider_id);
