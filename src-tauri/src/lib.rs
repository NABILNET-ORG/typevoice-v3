...
fn initialize_core_logic(app_handle: &AppHandle) -> anyhow::Result<()> {
    ...
    #[cfg(unix)]
    {
        match Signals::new(&[SIGUSR2]) {
            ...
            Err(e) => log::error!("Failed to set up signal handler: {}", e),
        }
    }
    ...
    let icon_resolved_path = app_handle
        .path()
        .resolve(&initial_icon_path, tauri::path::BaseDirectory::Resource)
        .context("Failed to resolve tray icon path")?;

    let icon = Image::from_path(icon_resolved_path)
...
pub fn run() {
    ...
    let specta_builder = Builder::<tauri::Wry>::new().commands(collect_commands![
        ...
        commands::get_post_process_api_key,
    ...
    builder
        ...
        .setup(move |app| {
            ...
            if let Err(e) = initialize_core_logic(&app_handle) {
                log::error!("Failed to initialize core logic: {}", e);
                return Err(e.into());
            }
